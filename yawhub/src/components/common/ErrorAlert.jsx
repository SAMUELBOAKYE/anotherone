...
const ErrorAlert = ({ 
  ...
  return (
    <div className={`error-alert ${currentType.className} ${isLeaving ? 'leaving' : ''}`} role="alert">
      {showIcon && (
      ...
      {dismissible && (
        <button className="error-close" onClick={handleClose} aria-label="Close alert">
      ...
      {autoDismiss && dismissible && (
        <div className="error-progress-bar">
          <div className="error-progress-fill" style={{ animationDuration: `${autoDismissTime}ms` }} />
        </div>
      )}
    </div>
  );
